...
class _MainScreenState extends State<MainScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        backgroundColor: _selectedIndex == 0 ? Colors.lightBlue.shade100 : null,
                            ...
                            child: ValueListenableBuilder<Map<String, int>>(
                              ...
                              builder: (context, moodMap, child) {
                                if (moodMap.containsKey(key)) {
                                  ...
                                  // 初期状態は Text('○') ではなく Icons.circle_outlined を表示
                                  return Icon(
  ...
  Widget _buildDateRow() {
    ...
    return Row(
      ...
      children: [
        ...
        // 編集ボタン（Tooltip付き）
        Tooltip(
          message: _isEditing ? '編集OFF' : '編集ON',
          child: IconButton(
            icon: Icon(_isEditing ? Icons.check : Icons.edit),
            onPressed: () {
              setState(() {
                if (_isEditing) {
                  // 編集モード終了時、ローカルの気分状態をグローバルに反映
                  String key =
                    '${_selectedDate.year}-${_selectedDate.month}-${_selectedDate.day}';
                  if (_localMood != null) {
